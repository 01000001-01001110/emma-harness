...
- **A `settings.json` written by PowerShell now starts Emma.** Every JSON config
  ...
  with an error that blamed a correct file. PowerShell's `>`, `Out-File` and
  `Set-Content -Encoding utf8` all write that mark, so a config generated by a
  script could not be used. The mark is ignored now.

- **One unreadable skill no longer costs you every skill.** A `SKILL.md` that is
  not valid UTF-8 took the whole session down, against the loader's own
  documented rule that such a file is skipped with a warning. It is skipped and
  counted now, in a `.claude/` library; an `.emma/` one still refuses, which is
  the existing difference between the two flavours. Commands behave the same
  way.

- **Skills in a nested directory are counted rather than silently ignored.**
  `skills/<group>/<name>/SKILL.md` — the shape a plugin ships — loaded as an
  empty catalogue with no explanation. `emma config check` now says how many
  were found and that only `skills/<name>/SKILL.md` is loaded.
