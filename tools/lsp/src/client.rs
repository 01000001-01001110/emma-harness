...
use crate::server::{Server, LANGUAGE_ID};
...
///   which only exist because build scripts and a build produced them. It goes
///   with the one above; enabling it alone does nothing.
///
/// `cargo.extraArgs: ["--offline"]` is the fourth, and it is what makes
/// `reaches_network: false` true by construction rather than by hope:
/// `cargo metadata` on a project with unfetched dependencies will otherwise
/// reach crates.io. Offline it fails loudly instead, which surfaces as an
/// unusable index rather than as silent egress.
///
...
/// the index. References *into* macro-generated code will be missed. That is a
/// real hole in exactly the direction this crate is most dangerous — a
/// confidently short answer — which is why it is in the tool descriptions and
/// not only here.
pub const INIT_OPTIONS: &str = r#"{
  "check": { "enable": false },
  "checkOnSave": false,
...
impl Client {
    ...
    async fn handshake(&self) -> Result<(), ToolError> {
        let init_options: Value =
            serde_json::from_str(INIT_OPTIONS).expect("INIT_OPTIONS is valid JSON");
        let params = json!({
...
mod tests {
    ...
    macro_rules! feed {
        ($msg:expr, $p:expr, $tx:expr, $otx:expr, $idx:expr, $ever:expr, $h:expr, $st:expr) => {
            handle($msg, &$p, &$tx, &$otx, &mut $idx, &mut $ever, &$h, &$st)
        };
    }
