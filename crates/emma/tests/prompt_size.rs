...
/// machine: no `Skill` (needs a harness), no `WebFetch`, `WebSearch` or browser
/// tools (need a Chrome). So this is a floor, and the comment below says by
/// roughly how much. The provider's optional search tool is not in any of these
/// numbers either: it is one small entry the provider appends on its own side of
/// `Request::tools`, and this test measures what Emma assembles.
