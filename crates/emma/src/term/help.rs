...
pub const SECTIONS: &[Section] = &[
    ...
    Section {
        ...
        entries: &[
            ...
            fixed("/compact", "summarise every finished goal but the last, now, instead of waiting for --max-context. It does not call a model, so it cannot take an instruction: it says so rather than ignoring the words. /compact all includes the last goal."),
            ...
            fixed("/resume", "says how to continue an earlier session, which has to happen before one starts: `emma --resume` on the command line, with an id to pick one by name."),
